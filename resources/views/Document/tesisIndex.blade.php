...
@section('headcontent') Data Tesis @endsection
    ...
    <div class="col-lg-12">
        <div class="card">
            <div class="card-header">
                <h3 class="card-title">Data Tesis</h3>
                @if (!empty($permissionAdd))
                    <a href="{{ url('doc/tesis/add') }}" class="btn btn-primary float-right btn-sm">
                        <i class="fas fa-plus mr-1"></i> Add Tesis
                    </a>
                @endif
            </div>
            <div class="card-body">
                <table id="tesisTable" class="table table-bordered table-striped table-hover">
                    <thead>
                        <tr>
                            <th>#</th>
                            <th>Judul</th>
...
<script>
    $(document).ready(function() {
        const startIndex = parseInt(localStorage.getItem("tesisPageStart")) || 0;

        const table = $('#tesisTable').DataTable({
            processing: true,
            serverSide: true,
        ...
        //     localStorage.setItem("tesisPageStart", pageStart);
        // });

        // Hapus posisi halaman jika user me-refresh langsung (tanpa klik tombol detail)
        if (!document.referrer || !document.referrer.includes('/doc/tesis/detail/')) {
            // localStorage.removeItem("tesisPageStart");
            table.state.clear();
        }
        ...
        $(document).on('click', '.delete-btn', function() {
            ...
            Swal.fire({
                ...
                if (result.isConfirmed) {
                    window.location.href = `{{ url('doc/tesis/delete/') }}/${id}`;
                }
            });
        });
    });
