...
<div class="col-md-8">
    ...
    <div class="skripsi-list">
        @foreach($tesis_data as $tesis)
        <div class="skripsi-item d-flex align-items-start">
            <img src="{{ asset('public/repo/img/' . $tesis->icon) }}" alt="File Icon">
            <div>
                <p class="skripsi-item-title position-relative">
                    <a href="{{ url('doc/detail/tesis/' . $tesis->document_id) }}" class="text-decoration-none text-dark">
                        {{ $tesis->title }}
                    </a>

                    @if ($tesis->file_path)
                        <a href="{{ url('doc/download/tesis/' . $tesis->document_id) }}" class="download-btn btn btn-success position-absolute" target="_blank">
                            <img src="{{ asset('public/repo/img/' . $tesis->icon) }}" alt="File Icon" class="file-icon"> Download Berkas
                        </a>
                    @else
                        &nbsp;<span class="text-danger position-absolute">Berkas kosong</span>
                    @endif
                </p>
                <p class="skripsi-item-meta">Oleh: {{ $tesis->nama_mahasiswa }} &nbsp;&nbsp; Tanggal: {{ date('d/m/Y', strtotime($tesis->upload_date)) }}</p>
            </div>
        </div>
        @endforeach
    </div>
        ...
        <div class="arsip-item d-flex align-items-center">
            ...
            <div>
                <p class="m-0">
                    ...
                    <br>
                    <small>Tesis tahun {{ $tahun->tahun_akademik }}: {{ $tahun->jumlah_data }} data</small>
                </p>
            </div>
        </div>
        @endforeach
